...
# Loads and saves memory.json automatically
# ============================================================
...
conversation_memory: dict[int, deque[dict[str, Any]]] = defaultdict(lambda: deque(maxlen=50))
...
def build_bot() -> discord.Client:
    ...
    async def chat(interaction: discord.Interaction, prompt: str, system: str | None = None) -> None:
        ...
        async with lock:
            ...
            persistent_memory["users"][str(interaction.user.id)] = {
...
def main() -> None:
    ...
    logger.info("Base URL: %s", os.getenv("LLM_BASE_URL"))
    logger.info("Memory file: memory.json")
    logger.info("================================")
